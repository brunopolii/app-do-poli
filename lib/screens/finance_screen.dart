...
class _FinanceScreenState extends State<FinanceScreen> {
  ...
  Future<void> _edit([MoneyTransaction? old]) async {
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    TextField(controller: amount, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Valor', prefixText: r'R$ ')),
                    SwitchListTile(
                      contentPadding: EdgeInsets.zero,
                      title: const Text('Entrada'),
                      value: isIncome,
                      onChanged: (value) { setDialogState(() { isIncome = value; category = 'Outros'; }); },
                    ),
    ...
    if (result != true || description.text.trim().isEmpty || parsed == null || parsed <= 0) return;
      ...
      child: ListView(
        ...
        children: <Widget>[
          ...
          AppCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
            ...
            Text(r'R$ ' + balance.toStringAsFixed(2), style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
            const SizedBox(height: 4),
            Text(r'Resultado do mês: R$ ' + (income - expense).toStringAsFixed(2)),
          ])),
          ...
          for (final item in current) AppCard(child: ListTile(
            contentPadding: EdgeInsets.zero,
            ...
            trailing: PopupMenuButton<String>(
              onSelected: (value) { if (value == 'edit') _edit(item); if (value == 'delete') _delete(item); },
  ...
  Widget _metric(String label, double value, IconData icon) {
    return AppCard(child: Row(children: <Widget>[
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
        Text(r'R$ ' + value.toStringAsFixed(2), style: const TextStyle(fontWeight: FontWeight.bold)),
        Text(label),
      ])),
    ]));
  }
