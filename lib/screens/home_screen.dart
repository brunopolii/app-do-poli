...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final todayWorkouts = plans.where((p) => p.weekdays.contains(now.weekday)).toList();
    ...
    final income = finance.where((e) => e.income).fold(0.0, (s, e) => s + e.amount);
    final expense = finance.where((e) => !e.income).fold(0.0, (s, e) => s + e.amount);
        ...
        children: [
          ...
          AppCard(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Seu dia', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
              const SizedBox(height: 16),
              Row(children: [
                _counter(Icons.event_outlined, 'Agenda', todayEvents.length.toString()),
                _counter(Icons.fitness_center, 'Treinos', todayWorkouts.length.toString()),
          ...
          AppCard(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (todayEvents.isEmpty) const Text('Nada agendado para hoje.')
              else ListTile(
                contentPadding: EdgeInsets.zero,
                leading: const CircleAvatar(child: Icon(Icons.event)),
                title: Text(todayEvents.first.title),
                subtitle: Text(todayEvents.first.start),
              ),
            ]),
          ),
          AppCard(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (goals != null) ...[
                ...
                LinearProgressIndicator(value: goals!['calories']! > 0 ? (calories / goals!['calories']!).clamp(0.0, 1.0).toDouble() : 0),
              ],
            ]),
          ),
          AppCard(
          ...
          if (todayWorkouts.isNotEmpty)
            AppCard(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                ...todayWorkouts.map((p) => ListTile(
                  contentPadding: EdgeInsets.zero,
                  leading: const CircleAvatar(child: Icon(Icons.fitness_center)),
                  title: Text(p.name),
                  subtitle: Text('${p.exercisesFor(now.weekday).length} exercícios'),
                )),
              ]),
            ),
        ],
  ...
  Widget _counter(IconData icon, String label, String value) {
    return Expanded(child: Column(children: [Icon(icon), const SizedBox(height: 6), Text(value, style: const TextStyle(fontWeight: FontWeight.bold)), Text(label, style: Theme.of(context).textTheme.bodySmall)]));
  }

  Widget _stat(String value, String label) {
    return Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(value, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)), Text(label, style: Theme.of(context).textTheme.bodySmall)]));
  }
}
