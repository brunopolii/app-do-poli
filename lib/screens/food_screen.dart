...
class _FoodScreenState extends State<FoodScreen> {
  ...
  Future<void> _setup() async {
    ...
    goals={'calories':kcal,'protein':w*2,'fat':kcal*.27/9,'carbs':(kcal-w*2*4-kcal*.27)/4};
    final prefs=await SharedPreferences.getInstance(); await prefs.setString('nutrition_goals','${goals!['calories']}|${goals!['protein']}|${goals!['carbs']}|${goals!['fat']}'); await _saveWeight(w); if(mounted)setState((){});
  }
  ...
  Future<void> _textAI() async { final c=TextEditingController(); final ok=await showDialog<bool>(context:context,builder:(ctx)=>AlertDialog(title:const Text('Analisar por texto'),content:TextField(controller:c,maxLines:3,decoration:const InputDecoration(hintText:'Ex.: 150g arroz e 100g frango')),actions:[TextButton(onPressed:()=>Navigator.pop(ctx,false),child:const Text('Cancelar')),FilledButton(onPressed:()=>Navigator.pop(ctx,true),child:const Text('Analisar'))])); if(ok!=true||c.text.trim().isEmpty)return; try{final result=await ai.estimateText(c.text);if(mounted)await _result(result);}catch(e){if(mounted)_message(e.toString());} }
