  ...
  @override Widget build(BuildContext context){
    ...
    return PopScope(canPop: false, onPopInvokedWithResult: (_, __) { Navigator.pop(context, ThemeEditResult(settings, mode)); }, child: Scaffold(appBar:AppBar(title:const Text('Personalizar tema'),leading:IconButton(icon:const Icon(Icons.close),onPressed:()=>Navigator.pop(context,ThemeEditResult(settings,mode)))),
      body:ListView(padding:const EdgeInsets.all(16),children:[
        ...
        _colorRow('Cards — preenchimento',card,()=>_pickColor('Cor do preenchimento dos cards',card,(c)=>_save(settings.copyWith(cardColorValue:c.toARGB32())))),
        _slider('Transparência do preenchimento',settings.cardOpacity,.05,1,(v)=>settings.copyWith(cardOpacity:v)),
        _colorRow('Cards — contorno',Color(settings.cardBorderColorValue),()=>_pickColor('Cor do contorno dos cards',Color(settings.cardBorderColorValue),(c)=>_save(settings.copyWith(cardBorderColorValue:c.toARGB32())))),
        _colorRow('Barra de navegação',nav,()=>_pickColor('Cor da barra de navegação',nav,(c)=>_save(settings.copyWith(navColorValue:c.toARGB32())))),
        _slider('Transparência da barra',settings.navOpacity,.05,1,(v)=>settings.copyWith(navOpacity:v)),
        _colorRow('Elementos de destaque',Color(settings.accentColorValue),()=>_pickColor('Cor dos elementos de destaque',Color(settings.accentColorValue),(c)=>_save(settings.copyWith(accentColorValue:c.toARGB32())))),
        const SizedBox(height:12),
