...
class _GymScreenState extends State<GymScreen> {
  ...
  Future<void> _savePlans() => StorageService.write(
        'workout_plans',
  ...
  List<List<String>> get library {
    ...
    final seen = <String>{};
    return all
        .where((e) => seen.add(e[0].trim().toLowerCase()))
        .toList();
  }
  ...
  Future<List<String>?> _newExercise() async {
              ...
              actions: [
                ...
                FilledButton(
                  onPressed: () {
                    ...
                    if (n.isNotEmpty) {
                      Navigator.pop(dialogContext, [n, muscle]);
                    }
                  },
    ...
    if (!customExercises.any(
      (e) => e[0].toLowerCase() == result[0].toLowerCase(),
    )) {
      customExercises.add(result);
      await StorageService.write(
        'exercise_library',
        customExercises
            .map((e) => {'name': e[0], 'muscle': e[1]})
            .toList(),
      );
    }
  ...
  Future<void> _createPlan([WorkoutPlan? old]) async {
    final draft = await Navigator.push<WorkoutPlanDraft>(
      context,
      MaterialPageRoute(
        builder: (_) => WorkoutPlanBuilder(
          initial: old,
          library: library,
          onCreateExercise: _newExercise,
        ),
      ),
    );
  ...
  Future<void> _start(WorkoutPlan plan, int day) async {
    ...
    if (exercises.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Este dia ainda não tem exercícios.')),
      );
      return;
    }
        ...
        children: [
                ...
                children: [
                  ...
                  for (final p in todayPlans)
                    ListTile(
                      ...
                      title: Text(p.name),
                      subtitle: Text(
                        '${p.exercisesFor(today).length} exercícios • ${weekDays[today - 1]}',
                      ),
                      trailing: FilledButton(
                        onPressed: () => _start(p, today),
          ...
          if (plans.isEmpty)
            const AppCard(
              child: Column(
                children: [
                  Icon(Icons.fitness_center_outlined, size: 42),
                  SizedBox(height: 8),
                  Text(
                  ...
                  SizedBox(height: 4),
                  Text(
                    'Crie seu primeiro treino e monte cada dia com os exercícios, séries e repetições que quiser.',
            ...
            AppCard(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 10),
                  Text(
                    '${p.weekdays.fold<int>(0, (sum, d) => sum + p.exercisesFor(d).length)} exercícios distribuídos nos dias',
...
class WorkoutPlanBuilder extends StatefulWidget {
  ...
  @override
  State<WorkoutPlanBuilder> createState() => _WorkoutPlanBuilderState();
}
...
class _WorkoutPlanBuilderState extends State<WorkoutPlanBuilder> {
  ...
  void initState() {
    ...
    if (widget.initial != null) {
      ...
      widget.initial!.dayExercises.forEach((k, v) {
        dayExercises[k] = v.map((e) => e.copy()).toList();
      });
    }
  }
  ...
  Future<void> _openDay(int day) async {
    final result = await Navigator.push<List<Exercise>>(
      context,
      MaterialPageRoute(
        builder: (_) => DayExerciseEditor(
          day: day,
          initial: (dayExercises['$day'] ?? [])
              .map((e) => e.copy())
              .toList(),
    ...
    if (result != null && mounted) {
      setState(() {
        ...
        dayExercises['$day'] = result;
      });
    }
  }

  void _save() {
    final valid = selectedDays
        .where((d) => (dayExercises['$d'] ?? []).isNotEmpty)
        .toSet();
    ...
    if (valid.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Adicione exercícios a pelo menos um dia.'),
        ),
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.initial == null ? 'Criar treino' : 'Editar treino'),
      ),
      body: ListView(
        ...
        children: [
          ...
          const Text(
            'Toque em um dia para abrir imediatamente a tela de exercícios.',
          ),
          const SizedBox(height: 12),
          for (int d = 1; d <= 7; d++)
            Padding(
              ...
              child: Card(
                child: ListTile(
                  ...
                  subtitle: Text(
                    (dayExercises['$d'] ?? []).isEmpty
                        ? 'Toque para montar este dia'
                        : '${dayExercises['$d']!.length} exercício(s) • toque para editar',
                  ),
...
class DayExerciseEditor extends StatefulWidget {
  ...
  @override
  State<DayExerciseEditor> createState() => _DayExerciseEditorState();
}
...
class _DayExerciseEditorState extends State<DayExerciseEditor> {
  ...
  Future<void> _add() async {
    ...
    if (result != null && mounted) setState(() => exercises.add(result));
  }

  Future<void> _edit(Exercise exercise) async {
    final result = await showDialog<Exercise>(
      context: context,
      builder: (_) => ExerciseSettings(initial: exercise),
    );
    if (result != null && mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(weekDays[widget.day - 1]),
        actions: [
          TextButton(
      ...
      body: ListView(
        ...
        children: [
          ...
          const Text(
            'Adicione exercícios e defina séries e repetições de cada um.',
          ),
          const SizedBox(height: 16),
          if (exercises.isEmpty)
            const AppCard(
              child: Column(
                children: [
                  Icon(Icons.playlist_add, size: 42),
                  SizedBox(height: 8),
                  Text(
                  ...
                  SizedBox(height: 4),
                  Text('Use o botão abaixo para adicionar.'),
                ],
              ),
            ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(exercises[i].muscle),
                          const SizedBox(height: 4),
                          Text(
                            '${exercises[i].sets} séries × ${exercises[i].reps} repetições',
                          ),
                        ],
                      ),
                    ),
                    IconButton(
                      onPressed: () => _edit(exercises[i]),
                      icon: const Icon(Icons.tune),
                    ),
                    IconButton(
                      onPressed: () => setState(() => exercises.removeAt(i)),
...
class _ExercisePickerState extends State<ExercisePicker> {
  ...
  void initState() {
    super.initState();
    items = widget.library.map((e) => [e[0], e[1]]).toList();
  }

  Future<void> _new() async {
    final result = await widget.onCreateExercise();
    if (result != null && mounted) {
      setState(() {
        items.removeWhere(
          (e) => e[0].toLowerCase() == result[0].toLowerCase(),
  ...
  Future<void> _add() async {
    ...
    final data = items.firstWhere((e) => e[0] == selected);
    final result = await showDialog<Exercise>(
      context: context,
      builder: (_) => ExerciseSettings(
        initial: Exercise(
          id: DateTime.now().microsecondsSinceEpoch.toString(),
          name: data[0],
          muscle: data[1],
  ...
  Widget build(BuildContext context) {
    final filtered = items
        .where(
          (e) =>
              e[0].toLowerCase().contains(query.toLowerCase()) ||
    ...
    return AlertDialog(
            ...
            Expanded(
              child: ListView(
                children: filtered
                    .map(
                      (e) => ListTile(
                        selected: selected == e[0],
                        leading: CircleAvatar(
                          child: Text(e[0].substring(0, 1).toUpperCase()),
                        ),
                        title: Text(e[0]),
                        subtitle: Text(e[1]),
                        onTap: () => setState(() => selected = e[0]),
                      ),
                    )
                    .toList(),
              ),
      ...
      actions: [
        TextButton(
          onPressed: _new,
          child: const Text('NOVO EXERCÍCIO'),
        ),
        TextButton(
  ...
  void _save() {
    Navigator.pop(
      ...
      Exercise(
        ...
        muscle: widget.initial.muscle,
        sets: sets,
        reps: reps,
        weights: widget.initial.weights,
...
class _WorkoutPlayerState extends State<WorkoutPlayer> {
  ...
  void initState() {
    super.initState();
    weightControllers = widget.exercises
        .map(
          (e) => e.weights
              .map(
                (w) => TextEditingController(text: w > 0 ? _fmt(w) : ''),
              )
              .toList(),
        )
        .toList();
  ...
  @override
  void dispose() {
    for (final row in weightControllers) {
      for (final c in row) {
        c.dispose();
      }
    }
  ...
  Future<void> _finish() async {
    ...
    for (var i = 0; i < widget.exercises.length; i++) {
      ...
      e.weights = List<double>.generate(
        e.sets,
        (s) => double.tryParse(
              weightControllers[i][s].text.replaceAll(',', '.'),
            ) ??
            0,
      );
                      ...
                      child: Row(
                        children: [
                          ...
                          SizedBox(
                            width: 90,
                            child: TextField(
                              controller: weightControllers[i][s],
                          ...
                          SizedBox(
                            width: 90,
                            child: Text('${widget.exercises[i].reps} reps'),
                          ),
                        ],
                      ),
                    ),
