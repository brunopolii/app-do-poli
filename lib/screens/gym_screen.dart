...
import '../widgets/gym_history_section.dart';
...
class _GymScreenState extends State<GymScreen> {
  ...
  List<List<String>> get library { final all = <List<String>>[...defaultExercises, ...customExercises]; final seen = <String>{}; return all.where((e) => seen.add(e[0].trim().toLowerCase())).toList(); }
  Future<List<String>?> _newExercise() async {
    final name = TextEditingController(); String muscle = muscles.last;
    final result = await showDialog<List<String>>(context: context, builder: (dialogContext) => StatefulBuilder(builder: (context, setDialogState) => AlertDialog(title: const Text('Novo exercício'), content: Column(mainAxisSize: MainAxisSize.min, children: [TextField(controller: name, autofocus: true, textCapitalization: TextCapitalization.sentences, decoration: const InputDecoration(labelText: 'Nome do exercício')), const SizedBox(height: 12), DropdownButtonFormField<String>(value: muscle, items: muscles.map((m) => DropdownMenuItem(value: m, child: Text(m))).toList(), onChanged: (v) { if (v != null) setDialogState(() => muscle = v); }, decoration: const InputDecoration(labelText: 'Grupo muscular'))]), actions: [TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('CANCELAR')), FilledButton(onPressed: () { final n = name.text.trim(); if (n.isNotEmpty) Navigator.pop(dialogContext, [n, muscle]); }, child: const Text('SALVAR'))])));
    name.dispose(); if (result == null) return null;
    if (!customExercises.any((e) => e[0].toLowerCase() == result[0].toLowerCase())) { customExercises.add(result); await StorageService.write('exercise_library', customExercises.map((e) => {'name': e[0], 'muscle': e[1]}).toList()); }
    if (mounted) setState(() {}); return result;
  }
  Future<void> _createPlan([WorkoutPlan? old]) async {
    ...
    plan.name = draft.name; plan.weekdays = draft.weekdays.toList()..sort(); plan.dayExercises = draft.dayExercises; plans.removeWhere((p) => p.id == plan.id); plans.add(plan); await _savePlans(); if (mounted) setState(() {});
  }
  ...
  @override Widget build(BuildContext context) {
    final today = DateTime.now().weekday; final todayPlans = plans.where((p) => p.weekdays.contains(today)).toList();
    return SafeArea(child: ListView(padding: const EdgeInsets.fromLTRB(16, 16, 16, 32), children: [
      ...
      for (final p in plans) AppCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [const CircleAvatar(child: Icon(Icons.fitness_center)), const SizedBox(width: 12), Expanded(child: Text(p.name, style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold))), PopupMenuButton<String>(onSelected: (v) async { if (v == 'edit') await _createPlan(p); if (v == 'delete') { plans.removeWhere((x) => x.id == p.id); await _savePlans(); if (mounted) setState(() {}); } }, itemBuilder: (_) => const [PopupMenuItem(value: 'edit', child: Text('Editar treino')), PopupMenuItem(value: 'delete', child: Text('Excluir treino'))])]), const SizedBox(height: 10), Wrap(spacing: 8, runSpacing: 8, children: p.weekdays.map((d) => ActionChip(label: Text(weekDays[d - 1]), onPressed: () => _start(p, d))).toList()), const SizedBox(height: 10), Text('${p.weekdays.fold<int>(0, (sum, d) => sum + p.exercisesFor(d).length)} exercícios distribuídos nos dias', style: Theme.of(context).textTheme.bodySmall)])),
    ]));
  }
}
...
class _WorkoutPlanBuilderState extends State<WorkoutPlanBuilder> {
  ...
  void _save() { final valid = selectedDays.where((d) => (dayExercises['$d'] ?? []).isNotEmpty).toSet(); if (name.text.trim().isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Dê um nome ao treino.'))); return; } if (valid.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Adicione exercícios a pelo menos um dia.'))); return; } Navigator.pop(context, WorkoutPlanDraft(name: name.text.trim(), weekdays: valid, dayExercises: dayExercises)); }
  @override Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: Text(widget.initial == null ? 'Criar treino' : 'Editar treino')), body: ListView(padding: const EdgeInsets.fromLTRB(16, 8, 16, 32), children: [TextField(controller: name, decoration: const InputDecoration(labelText: 'Nome do treino', prefixIcon: Icon(Icons.edit_outlined))), const SizedBox(height: 24), Text('Escolha os dias', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)), const SizedBox(height: 4), const Text('Toque em um dia para abrir imediatamente a tela de exercícios.'), const SizedBox(height: 12), for (int d = 1; d <= 7; d++) Padding(padding: const EdgeInsets.only(bottom: 8), child: Card(child: ListTile(leading: CircleAvatar(child: Text('$d')), title: Text(weekDays[d - 1], style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text((dayExercises['$d'] ?? []).isEmpty ? 'Toque para montar este dia' : '${dayExercises['$d']!.length} exercício(s) • toque para editar'), trailing: const Icon(Icons.chevron_right), onTap: () => _openDay(d)))), const SizedBox(height: 12), FilledButton.icon(onPressed: _save, icon: const Icon(Icons.save_outlined), label: const Text('SALVAR TREINO'), style: FilledButton.styleFrom(minimumSize: const Size.fromHeight(52)))]));
}

class DayExerciseEditor extends StatefulWidget { final int day; final List<Exercise> initial; final List<List<String>> library; final Future<List<String>?> Function() onCreateExercise; const DayExerciseEditor({super.key, required this.day, required this.initial, required this.library, required this.onCreateExercise}); @override State<DayExerciseEditor> createState() => _DayExerciseEditorState(); }
class _DayExerciseEditorState extends State<DayExerciseEditor> {
  late List<Exercise> exercises; @override void initState() { super.initState(); exercises = widget.initial; }
  ...
  @override Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: Text(weekDays[widget.day - 1]), actions: [TextButton(onPressed: () => Navigator.pop(context, exercises), child: const Text('SALVAR'))]), body: ListView(padding: const EdgeInsets.fromLTRB(16, 12, 16, 32), children: [Text('Exercícios', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)), const Text('Adicione exercícios e defina séries e repetições de cada um.'), const SizedBox(height: 16), if (exercises.isEmpty) const AppCard(child: Column(children: [Icon(Icons.playlist_add, size: 42), SizedBox(height: 8), Text('Nenhum exercício neste dia', style: TextStyle(fontWeight: FontWeight.bold)), SizedBox(height: 4), Text('Use o botão abaixo para adicionar.') ])), for (int i = 0; i < exercises.length; i++) Card(child: Padding(padding: const EdgeInsets.all(12), child: Row(children: [CircleAvatar(child: Text('${i + 1}')), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(exercises[i].name, style: const TextStyle(fontWeight: FontWeight.bold)), Text(exercises[i].muscle), const SizedBox(height: 4), Text('${exercises[i].sets} séries × ${exercises[i].reps} repetições')])), IconButton(onPressed: () => _edit(exercises[i]), icon: const Icon(Icons.tune)), IconButton(onPressed: () => setState(() => exercises.removeAt(i)), icon: const Icon(Icons.delete_outline))]))), const SizedBox(height: 8), OutlinedButton.icon(onPressed: _add, icon: const Icon(Icons.add), label: const Text('ADICIONAR EXERCÍCIO'), style: OutlinedButton.styleFrom(minimumSize: const Size.fromHeight(50))) ]));
}

class ExercisePicker extends StatefulWidget { final List<List<String>> library; final Future<List<String>?> Function() onCreateExercise; const ExercisePicker({super.key, required this.library, required this.onCreateExercise}); @override State<ExercisePicker> createState() => _ExercisePickerState(); }
...
class _ExerciseSettingsState extends State<ExerciseSettings> {
  late int sets; late int reps; @override void initState() { super.initState(); sets = widget.initial.sets; reps = widget.initial.reps; }
...
class _WorkoutPlayerState extends State<WorkoutPlayer> {
  ...
  @override void initState() { super.initState(); weightControllers = widget.exercises.map((e) => e.weights.map((w) => TextEditingController(text: w > 0 ? _fmt(w) : '')).toList()).toList(); }
  String _fmt(double v) => v % 1 == 0 ? v.toInt().toString() : v.toString();
  @override void dispose() { for (final row in weightControllers) { for (final c in row) c.dispose(); } super.dispose(); }
  Future<void> _finish() async {
    final completed = <Exercise>[];
    for (var i = 0; i < widget.exercises.length; i++) {
      final e = widget.exercises[i].copy();
      e.weights = List<double>.generate(e.sets, (s) => double.tryParse(weightControllers[i][s].text.replaceAll(',', '.')) ?? 0);
      e.done = List<bool>.filled(e.sets, true);
      completed.add(e);
    }
  ...
  @override Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: Text(widget.name)), body: ListView(padding: const EdgeInsets.all(16), children: [Text('Treino em execução', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)), const SizedBox(height: 12), for (int i = 0; i < widget.exercises.length; i++) AppCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(widget.exercises[i].name, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)), Text(widget.exercises[i].muscle), const SizedBox(height: 8), for (int s = 0; s < widget.exercises[i].sets; s++) Padding(padding: const EdgeInsets.only(bottom: 8), child: Row(children: [Text('Série ${s + 1}'), const Spacer(), SizedBox(width: 90, child: TextField(controller: weightControllers[i][s], keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(hintText: 'kg'))), const SizedBox(width: 8), SizedBox(width: 90, child: Text('${widget.exercises[i].reps} reps'))]))])), const SizedBox(height: 8), FilledButton(onPressed: _finish, child: const Text('FINALIZAR TREINO'))]));
}
