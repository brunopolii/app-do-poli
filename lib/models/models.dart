class Exercise {
  ...
  Exercise copy() => Exercise(id: id, name: name, muscle: muscle, sets: sets, reps: reps, weights: [...weights], done: [...done]);

  Map<String, dynamic> toJson() => {'id': id, 'name': name, 'muscle': muscle, 'sets': sets, 'reps': reps, 'weights': weights, 'done': done};
  factory Exercise.fromJson(Map<String, dynamic> j) => Exercise(id: j['id'] as String, name: j['name'] as String, muscle: (j['muscle'] as String?) ?? '', sets: (j['sets'] as num?)?.toInt() ?? 3, reps: (j['reps'] as num?)?.toInt() ?? 10, weights: ((j['weights'] as List?) ?? const []).map((e) => (e as num).toDouble()).toList(), done: ((j['done'] as List?) ?? const []).map((e) => e == true).toList());
}
...
class Workout {
  ...
  factory Workout.fromJson(Map<String, dynamic> j) { final date=(j['date'] as String?)??''; int day=(j['weekday'] as num?)?.toInt()??0; if(day<1||day>7){try{day=DateTime.parse(date).weekday;}catch(_){day=1;}} return Workout(id:j['id'] as String,name:j['name'] as String,date:date,weekday:day,exercises:((j['exercises'] as List?)??const[]).map((e)=>Exercise.fromJson(Map<String,dynamic>.from(e as Map))).toList()); }
}
...
class WorkoutPlan {
  ...
  WorkoutPlan({required this.id, required this.name, required this.weekdays, required this.dayExercises});
  List<Exercise> exercisesFor(int day) => dayExercises['$day'] ?? const [];
  Map<String,dynamic> toJson()=>{'id':id,'name':name,'weekdays':weekdays,'dayExercises':dayExercises.map((k,v)=>MapEntry(k,v.map((e)=>e.toJson()).toList()))};
  factory WorkoutPlan.fromJson(Map<String,dynamic> j){
    final raw=Map<String,dynamic>.from((j['dayExercises'] as Map?)??{});
    return WorkoutPlan(id:(j['id']??'').toString(),name:(j['name']??'Treino').toString(),weekdays:((j['weekdays'] as List?)??const[]).map((e)=>(e as num).toInt()).toList(),dayExercises:raw.map((k,v)=>MapEntry(k,((v as List?)??const[]).map((e)=>Exercise.fromJson(Map<String,dynamic>.from(e as Map))).toList())));
  }
}
...
class AgendaEvent {
  ...
  Map<String,dynamic> toJson()=>{'id':id,'date':date,'title':title,'description':description,'start':start,'end':end,'notify':notify};
  factory AgendaEvent.fromJson(Map<String,dynamic> j)=>AgendaEvent(id:j['id'] as String,date:j['date'] as String,title:j['title'] as String,description:(j['description'] as String?)??'',start:(j['start'] as String?)??'08:00',end:(j['end'] as String?)??'09:00',notify:j['notify']==true);
}
...
class WeightEntry {
  ...
  factory WeightEntry.fromJson(Map<String,dynamic> j)=>WeightEntry(date:j['date'] as String,weight:(j['weight'] as num).toDouble());
}
